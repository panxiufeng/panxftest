...
import com.pxf.project.common.util.StringUtil;
...
public class ValidateUtil {
	private static Pattern phone = Pattern
			.compile("^0?(13|15|18|14|17)[0-9]{9}$");
	private static Pattern password = Pattern
			.compile("^(?!\\D+$)(?![^a-zA-Z]+$)\\S{6,16}$");
	...
	private static Pattern telRex = Pattern
			.compile("^([0-9]{3,4}-)?[0-9]{7,8}$");
	private static Pattern payPwd = Pattern.compile("^(\\d{6})$");
	private static Pattern chineseChar = Pattern
			.compile("^([\u4e00-\u9fa5]{1,})$");
	private static Pattern annualIncome = Pattern
			.compile("(^[\\d]{1,9}(\\.[\\d]{0,2})?)$");

	public static String[] illegalStrs = { "insert", "select ", "drop ",
			"update ", "delete ", "java", "javascript", "script", "<script",
			"<input", "alert ", "onerror", "onclick", "onload", "iframe",
			"frame" };
